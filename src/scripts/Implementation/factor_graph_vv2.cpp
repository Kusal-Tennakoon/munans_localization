...
// std::vector<std::vector<int>> feedback{{2,2},{4,4},{6,6},{8,8},{10,10},{14,14},{16,16},{18,18},{20,20},{22,22},{24,24},{26,26},{28,28},{30,30},{32,32},{36,36},{38,38},{40,40},{44,44}};
...
//0) Parameters
    ...
    const double rctx = 0.5; // x translation of the robot's camera w.r.t the robot
    const double rcty = 0; // y translation of the robot's camera w.r.t the robot
    const double rctz = 0.458-ground_clearance; // z translation of the robot's camera w.r.t the robot
    const double rcthx = -pi/2; // x rotation of the robot's camera w.r.t the robot
    const double rcthy = 0; // y rotation of the robot's camera w.r.t the robot
    const double rcthz = -pi/2; // z rotation of the robot's camera w.r.t the robot

    const double mctx = 0; // x translation of the robot's camera w.r.t the robot
    const double mcty = 0; // y translation of the robot's camera w.r.t the robot
    const double mctz = h_tripod; // z translation of the robot's camera w.r.t the robot
    const double mcthx = -pi/2; // x rotation of the robot's camera w.r.t the robot
    const double mcthy = 0; // y rotation of the robot's camera w.r.t the robot
    const double mcthz = -pi/2; // z rotation of the robot's camera w.r.t the robot		
    ...
    const double evx = 0.001 ; //Noise in visual x
    const double evy = 0.001; // Noise in visual y
    const double evz = 0.001; // Noise in visual z
    const double evthx = 0.001*pi/6; //Noise in visual theta about x-axis
    const double evthy = 0.001*pi/6; //Noise in visual theta about y-axis
    const double evthz = 0.001*pi/6; //Noise in visual theta about z-axis. This should be a high value

	const double ecx = 0.01 ; //Noise in robot/map to camera translation x
    const double ecy = 0.01; // Noise in robot/map to camera translation y
    const double ecz = 0.01; // Noise in robot/map to camera translation z
    const double ecthx = 0.01*pi/180; //Noise in robot/map to camera rotation about x-axis
    const double ecthy = 0.01*pi/180; //Noise in robot/map to camera rotation about y-axis
    const double ecthz = 0.001*pi/180; //Noise in robot/map to camera rotation about z-axis.
	...
	noiseModel::Diagonal::shared_ptr camNoise = noiseModel::Diagonal::Sigmas((Vector(6) << ecthx,ecthy,ecthz,ecx,ecy,ecz).finished());
	...
	std::vector<Pose3> rob_cam_poses;
	std::vector<Pose3> map_cam_poses;
...
munans::errorStats computeErrorStats(std::vector<Pose3> result_poses,std::vector<Pose3> traj_poses){
	...
	std::vector<float> pos_errors, rot_errors;
	float delta_x = 0,delta_y = 0,delta_z = 0,delta_t = 0,delta_th = 0,dt2 = 0,dth2 = 0;
	float dt_tot = 0;
	float dth_tot = 0;
	// Matrix33 delta_R;
...
// 5) Print result
  ...
  // ---------------------------------
		...
		Visualizer::publishErrorStats(errorStats);
...
void trajectory_callback(const geometry_msgs::PoseStamped::ConstPtr& msg){
    ...
    if ((visual_node_id <= feedback.size()) && (traj_node_id == feedback.at(visual_node_id-1).at(0))){
        ...
        Pose3 rob_node = accel_poses.at(feedback.at(visual_node_id-1).at(0)-1);
        Pose3 map_node = Map_poses.at(feedback.at(visual_node_id-1).at(1)-1);

		Rot3 Rrc = Rot3::RzRyRx(rcthx,rcthy,rcthz); // Orientation of the robot's camera w.r.t the robot
		Rot3 Rmc = Rot3::RzRyRx(mcthx,mcthy,mcthz); // Orientation of the map camera w.r.t the map;

		Point3 trc = Point3(rctx,rcty,rctz); // Translation of the robot's camera w.r.t the robot
		Point3 tmc = Point3(mctx,mcty,mctz); // Translation of the robot's camera w.r.t the robot		

		Pose3 rob_cam = rob_node * Pose3(Rrc,trc); // Pose of robot's camera w.r.t to the odometer
		Pose3 map_cam = map_node * Pose3(Rmc,tmc);	
		Pose3 rob_cam_actual = traj_poses.at(feedback.at(visual_node_id-1).at(0)-1) * Pose3(Rrc,trc);	

		rob_cam_poses.push_back(rob_cam);
		map_cam_poses.push_back(map_cam);

		initial.insert(Symbol('C',rob_cam_id),rob_cam);
		initial.insert(Symbol('D',map_cam_id),map_cam);

		graph.emplace_shared<BetweenFactor<Pose3> >(Symbol('r',feedback.at(visual_node_id-1).at(0)), Symbol('C',rob_cam_id), Pose3(Rrc,trc), camNoise);				
		graph.emplace_shared<BetweenFactor<Pose3> >(Symbol('m',feedback.at(visual_node_id-1).at(1)), Symbol('D',map_cam_id), Pose3(Rmc,tmc), camNoise);
		
		Visualizer::collectCameraPose(rob_cam,"rob_cam" + std::to_string(rob_cam_id),cam_no);
		Visualizer::collectCameraPose(map_cam,"vis_fbk_" + std::to_string(map_cam_id),cam_no);
		Visualizer::publishCameraPose();

        Pose3 visual_pose = rob_cam_actual.inverse() * map_cam;	
        Visualizer::collectVisualFeedback(rob_cam_poses.at(rob_cam_id-1),map_cam_poses.at(map_cam_id-1),arrow_no);
    ...
    // 3.7.4) Adding the Essential Matrix Constraint
        ...
        graph.emplace_shared<EssentialMatrixConstraint>(Symbol('C',rob_cam_id),Symbol('D',map_cam_id),EssentialMat,visualNoise);
        ...
        Pose3 rob_node = accel_poses.at(virtual_views.at(vv_id-1).at(0)-1);
        Pose3 map_node = Map_poses.at(virtual_views.at(vv_id-1).at(1)-1);
		...
		Pose3 rob_cam = rob_node * Pose3(Rrc,trc); // Pose of robot's camera w.r.t to the odometer
		Pose3 map_cam = map_node * Pose3(Rmc,tmc);	
		Pose3 rob_cam_actual = traj_poses.at(virtual_views.at(vv_id-1).at(0)-1) * Pose3(Rrc,trc);	
		...
		graph.emplace_shared<BetweenFactor<Pose3> >(Symbol('r',virtual_views.at(vv_id-1).at(0)), Symbol('C',rob_cam_id), Pose3(Rrc,trc), camNoise);				
		graph.emplace_shared<BetweenFactor<Pose3> >(Symbol('m',virtual_views.at(vv_id-1).at(1)), Symbol('D',map_cam_id), Pose3(Rmc,tmc), camNoise);
		
		Visualizer::collectCameraPose(rob_cam,"rob_cam" + std::to_string(rob_cam_id),cam_no);
		Visualizer::collectCameraPose(map_cam,"vis_fbk_" + std::to_string(map_cam_id) + "(VV)",cam_no);
		Visualizer::publishCameraPose();

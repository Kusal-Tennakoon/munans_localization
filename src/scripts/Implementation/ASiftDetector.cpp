...
void ASiftDetector::detectAndCompute(const Mat& img, std::vector< KeyPoint >& keypoints, Mat& descriptors)
{
  ...
  for(int tl = 1; tl < 6; tl++)
  {
    ...
    for(int phi = 0; phi < 180; phi += 72.0/t)
    {
      ...
      // Ptr<Feature2D> sift = xfeatures2d::SIFT::create(50,3,0.09,2,2);
      Ptr<Feature2D> sift = xfeatures2d::SIFT::create(100,3,0.1,2,1.6);

      sift->detect(timg, kps, mask);
      sift->compute(timg, kps, desc);

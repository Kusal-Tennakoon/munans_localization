...
// std::vector<std::vector<int>> feedback{{3,3},{5,5},{7,7},{9,9},{11,11},{13,13},{15,15},{17,17},{19,19},{21,21},{23,23},{25,25},{27,27},{29,29},{31,31},{33,33},{35,35},{37,37},{39,39},{41,41},{43,43},{45,45}};
...
//0) Parameters
    ...
    const double ex = 0.1; //Odometry noise in x
    const double ey = 0.1; //Odometry noise in y
    const double ez = 0.1; //Odometry noise in y
...
void trajectory_callback(const geometry_msgs::PoseStamped::ConstPtr& msg){
	...
	if (visual_node_id <= feedback.size()){
		if (traj_node_id == feedback.at(visual_node_id-1).at(0)){
			...
			// Rot3 wRm = map_node.rotation();
			// Rot3 wRr = rob_node.rotation();
			// Point3 wtm = map_node.translation();
			// Point3 wtr = rob_node.translation() ;
			// Rot3 temp = wRr.inverse(); 
			// Rot3 rRm = temp.operator*(wRm);
			// Point3 rtm = wRr.unrotate(wtm.vector()-wtr.vector());
			Pose3 visual_pose = rob_node.inverse() * map_node;

			// Pose3 visual_pose = Pose3(rRm,rtm);
...
void odometer_callback(const geometry_msgs::PoseStamped::ConstPtr& msg){
	...
	if ((odom_node_id >1)){
		graph.emplace_shared<BetweenFactor<Pose3> >(Symbol('r',odom_node_id-1), Symbol('r',odom_node_id), odom_poses.at(odom_node_id-2), odomNoise);
...
// 	if ((correction == "on") && (visual_node_id >= 1) &&((visual_node_id)%feedback_interval)==0){
...
// 	    // std::string capture_img = "./src/Image_Data/Images/node(" + std::to_string(visual_node_id) + ")_crop.jpg";
...
// 			// Point3 trans_vec = Point3(map[i+1].pose.x-robot[i].pose.x	}else{
